  ...
  return (
    <nav className="w-full flex justify-between items-center bg-[#0d1429] flex-row mb-2">
      <div className="flex md:w-[20%] w-full flex-initial justify-start items-center cursor-pointer">
        <Link to={{pathname: "/"}}>
          <img src={logo} alt="logo" className="object-contain w-full cursor-pointer px-3 md:mr-1" />
        </Link>
      </div>
      <Link to={'/'}>
        ...
        <div className="flex justify-start ml-2">
          ...
          <Link to={'/'}>
            <li className={"flex justify-center items-center px-4 h-full cursor-pointer transition hover:duration-700 hover:bg-gray-600 rounded-sm" 
              ...
              <AiOutlineShop className="mr-2"/>Marketplace
            </li>
          </Link>
          <Link to={'/'}>
            <li className="flex justify-center items-center px-4 h-full cursor-pointer transition hover:duration-700 hover:bg-gray-600 rounded-sm">
              <HiOutlineTicket className="mr-2"/>Events
            </li>
          </Link>
        </div>
      </ul>
        {!toggleMenu && (
            <HiMenuAlt4 fontSize={28} className="justify-end items-end w-full text-white md:hidden cursor-pointer" onClick={() => setToggleMenu(true)} />
        )}
